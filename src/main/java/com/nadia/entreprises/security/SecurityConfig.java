...
public class SecurityConfig {
	...
	SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {
		http.authorizeHttpRequests((requests) -> requests.requestMatchers("/showCreate", "/saveEntreprise")
				.hasAnyAuthority("ADMIN", "AGENT").requestMatchers("/modifierEntreprise", "/supprimerEntreprise")
				.hasAuthority("ADMIN").requestMatchers("/listeEntreprises").hasAnyAuthority("ADMIN", "AGENT", "USER")
	...
	*/
	 ...
	 * authorities("ADMIN") .build(); UserDetails userNadia =
	 * User.withUsername("nadia").password(passwordEncoder.encode("123"))
	 * .authorities("AGENT", "USER").build(); UserDetails user1 =
	 * User.withUsername("user1").password(passwordEncoder.encode("123")).
	 * authorities("USER") .build();
	 * 
	 * return new InMemoryUserDetailsManager(admin, userNadia, user1);
	 * 
	 * }
	 */
